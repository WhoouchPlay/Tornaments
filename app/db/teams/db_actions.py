...
async def remove_team(team_id: str, user_id: str, db: AsyncSession):
...
async def add_user_to_team_by_teamlead(team_id: str, user_id: str, member_user_id: str, db: AsyncSession) -> bool:
    user_team_assoc = await db.scalar(select(Team).filter(Team.id==team_id, User.id==user_id, UserTeamAssoc.role==Role.teamlead))
    user: Optional[User] = await get_user(user_id=user_id, db=db)

    if not user_team_assoc or not user:
        return False
    
    user_team_assoc.users.users.append(user)
    await db.commit()
    return True
