...
class User(Base):
    ...
    def if_verify_password(self, pwd: str):
        return bcrypt.checkpw(pwd.encode(), self.password_.encode())
    
    def get_token(self, pwd: str, expire_time_minutes: str = settings.exp_time_minutes) -> str:
        if not self.if_verify_password(pwd):
            return

        payload = dict(user_id=self.id, exp=datetime.now(timezone.utc) + timedelta(expire_time_minutes))
